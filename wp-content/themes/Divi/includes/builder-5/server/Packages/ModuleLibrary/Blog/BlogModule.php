...
class BlogModule implements DependencyInterface {
	...
	/**
	 * Keep Blog Read More hover behavior scoped to the link target.
	 *
	 * For hover-state custom CSS, append `:hover` to the read-more link selector so
	 * frontend output targets only the hovered link. For sticky state, retain
	 * `.et_pb_sticky` targeting on the module selector.
	 *
	 * @since ??
	 *
	 ...
	 * @return string
	 */
	public static function read_more_custom_css_selector_function( array $params ): string {
		$selector = (string) ( $params['selector'] ?? '' );
		$state    = (string) ( $params['state'] ?? 'value' );

		if ( ! str_contains( $selector, ' a.more-link' ) ) {
			return $selector;
		}
		...
		if ( 'sticky' === $state ) {
			...
			$sticky_selectors = array_map(
				static function ( string $item ): string {
					...
					$parent_selector = implode( ' ', array_slice( $parts, 0, -1 ) );
					$child_selector  = $parts[ count( $parts ) - 1 ];
					$sticky_parent   = str_contains( $parent_selector, '.et_pb_sticky' )
						? $parent_selector
						: $parent_selector . '.et_pb_sticky';

					return $sticky_parent . ' ' . $child_selector;
				},
				$selectors
			);
		...
		$link_hover_selectors = array_map(
			static function ( string $item ): string {
		...
		return implode( ', ', $link_hover_selectors );
	}
				...
				'styles'        => [
					...
					CssStyle::style(
						[
							...
							'selectorFunction'       => [ self::class, 'read_more_custom_css_selector_function' ],
							'orderClass'             => $order_class,
							'isInsideStickyModule'   => $is_inside_sticky_module,
							'stickyParentOrderClass' => $sticky_parent_order_class,
						]
